...
Despite this, a review of the medical deepfake detection literature reveals that every existing dataset and method targets \emph{human} imaging modalities. MedForensics, presented at MICCAI 2025, spans six human modalities (ultrasound, endoscopy, histopathology, MRI, CT, X-ray) and twelve generative models, explicitly measuring cross-generator generalization \cite{medforensics2025}. M3Dsynth addresses AI-generated local manipulations in human 3D medical volumes \cite{m3dsynth2023}. Shahzad et al., comparing GAN- and diffusion-generated fakes directly, report the same generalization gap using human skin-cancer imagery \cite{hybrid_gan_diffusion_2026}. None of this work, nor any dataset we could locate on Kaggle or elsewhere, considers veterinary diagnostic images.
...
MedForensics is, to our knowledge, the most comprehensive published benchmark for medical deepfake detection, spanning six modalities and twelve generative models, and proposes DSKI, a dual-stage knowledge-infusing detector built on a vision-language feature space that outperforms prior methods and human experts in a visual Turing test \cite{medforensics2025}. Its architecture uses three parallel CNN streams targeting visual perception, resampling artifacts, and local inconsistency. M3Dsynth focuses on localized manipulations within 3D volumes rather than whole-image synthesis \cite{m3dsynth2023}. Shahzad et al. train detectors on skin-cancer imagery against both GAN (DCGAN, Pix2Pix) and diffusion (DDPM, VQ-Diffusion) generators, confirming that detectors calibrated to one generator family degrade sharply against the other, and propose domain-specific batch normalization to mitigate this distribution shift \cite{hybrid_gan_diffusion_2026}.
...
All experiments were run on Apple silicon (M4 Pro) using PyTorch's Metal Performance Shaders (MPS) backend; no CUDA hardware was used. The generator was trained for 800 epochs on the 152 real images (batch size 16, Adam, lr $=2\times10^{-4}$), producing a pool of 2{,}300 synthetic radiographs, of which 456 (three times the real count) were sampled for detector training to bound class imbalance while the full pool is released on Kaggle for reuse. Images were resized to $128\times128$ for both the generator and the detector. The combined dataset (152 real + 456 synthetic images, 608 total) was split 64/16/20 into train/validation/test (388/98/122) with stratified sampling on the real/fake label, using a fixed random seed (42) for reproducibility. The detector was trained with Adam (lr $=1\times10^{-4}$) for 25 epochs, selecting the checkpoint with the highest validation AUC for final test-set evaluation. Code, exact hyperparameters, and the trained generator/detector weights accompany this paper in the project repository.

% -------------------------------------------------------
\section{Results}
\label{sec:results}

On the held-out test set (122 images: 30 real, 92 synthetic), the dual-stream detector reaches 100\% accuracy, precision, recall, F1, and AUC (Table~\ref{tab:results}). Validation AUC reached 1.0 by epoch 1 and training loss converged to near zero within 20 epochs, indicating the classification task was fully separable for this generator.

\begin{table}[htbp]
\centering
\caption{Test-set performance of the dual-stream detector}
\label{tab:results}
\begin{tabular}{lc}
\toprule
Metric & Value \\
...
We interpret this ceiling result cautiously rather than as evidence of a solved problem: a DCGAN trained on only 152 images at $128\times128$ resolution leaves generation artifacts (Figure~\ref{fig:freq}) that are far more separable than those of a modern diffusion model would be. Section~\ref{sec:discussion} details why we treat this as a lower bound on task difficulty, not an upper bound on detector capability.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/roc_curve.png}
  \caption{ROC curve of the dual-stream detector on the held-out test set. The curve saturates at AUC = 1.0, consistent with the full separability discussed in the text.}
  \label{fig:roc}
\end{figure}
...
\begin{figure}[htbp]
  ...
  \caption{Average log-power spectrum of real radiographs (left), generated radiographs (center), and their difference (right), computed over the full 152 real and 2{,}300 generated images. The periodic structure visible in the difference map corresponds to the transposed-convolution upsampling artifacts described in Section~\ref{sec:related}.}
  \label{fig:freq}
\end{figure}
...
\textbf{Interpreting the 100\% result.} A perfect test-set score is a warning sign as much as a result. With only 152 training images, our DCGAN converges to a generator whose outputs remain visually and spectrally distinguishable from real radiographs at essentially every frequency band, so the detection task is closer to distinguishing two different image distributions than to catching a near-miss forgery. We report it transparently rather than omit it, but we explicitly do not claim this generalizes to stronger generators. The correct reading is: our dual-stream architecture is a sound baseline, and the interesting open question, does it still separate real from fake once the generator is a fine-tuned diffusion model rather than a small DCGAN, remains unanswered and is the paper's most direct future-work item.
